...
WSN and AMQP use different behaviours for handling topics/queues. As WSN is a pure publish/subscribe protocol, it is only concerned with topics. This means that that each subscriber to a topic will receive messages sent on the topic. Queues are only concerned with delivering a message that is currently in queue, not what client that receives it. This can be viewed as a kind of load balancing across for example multiple servers.
