...
The group received a version of NFFIPlayer\footnote{\ref{subsec:prestudies-existing_solutions-micro_wsn_and_nffiplayer}} with AMQP support from FFI which implemented the 0.9.1 standard. However due to the significant changes in the standard, it could not be used to any extent.

To be able to test the implementation properly, the group found two other AMQP 1.0 Java implementations and wrote simple scripts to test the both send and receive functionality. As there is a limited amount of free/open implementations, the group ended up using one open source from Apache Qpid (not proton)\footnote{\url{https://qpid.apache.org}} and one commercial from SwiftMQ\footnote{\url{http://www.swiftmq.com}}.

By using implementations that where not based on the Qpid Proton library, the group had a better basis for stating the correctness of the implementation and the quality of the product.
