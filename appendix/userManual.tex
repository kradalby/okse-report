...
For the JRE to function properly, it must be added to the path variable. On UNIX-based systems, like Linux or OS X, it is done by entering the following command in the terminal: 
\begin{verbatim}
export JAVA_HOME=<PATH-TO-JRE>
\end{verbatim}
On windows, open up a command line window (cmd.exe) and type in:
\begin{verbatim}
...
Download the latest version of OKSE Message Broker from http://okse.fap.no/ or use the version provided with this document. The files can be placed at any location within the filesystem, as long as the user has write permission in the target folder. The developers recommends to add all the files in a folder called OKSE, within the users home folder.

\subsection{Running the software}

On Windows, the broker can be started by double-clicking on the .jar-file. On UNIX-based systems and Windows alike, the broker is started by running the command: 
\begin{verbatim}
java -jar <PATH>/<JAR-FILE>.jar
\end{verbatim}

\noindent from the command line or terminal. If the broker is going to be used for bigger files or a very high amount of messages, a higher memory limit should be set for Java, this can be done by using the following command: 
\begin{verbatim}
java -jar -Xms2048m -Xmx4096m <PATH>/<JAR-FILE>.jar
\end{verbatim}
...
\setlength{\itemsep}{0cm}%
    ...
    \item Open cmd.exe,
    \item Change directory to the C:/Python27 folder. 
...
\noindent If you get command not found, consult your package manager on how to install Python 2.7 or go to python.org and download the installer from there.
...
\noindent After the installation, change directory in the terminal to the location of bullrider.py.
...
\begin{verbatim}
python bullrider.py notify localhost 61000 example
\end{verbatim}
